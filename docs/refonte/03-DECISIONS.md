...
| 2026-09-05 (implémentation Phase 8A) | **R (précision — migration Storage non anticipée) ; statut Phase 8A** | Implémentation de la Phase 8A (médias FICHE cloud : `SupabaseMediaRepository`, hook `useFicheMedia()`, migration de `FicheDetail`/`FabricPhotos`/`VoiceRecorder`/`SignaturePad` vers `MediaRepository` — `Fiche.tissuPhotos`/`voiceNote`/`signature` deviennent volontairement non autoritatifs dès qu'un Repository média existe, voir `mappers/fiche.ts`). **Correction factuelle** à la documentation antérieure (« Phase 8A : aucune migration SQL ») : le schéma métier `public.media_assets` (Phase 2) est intact, mais **`storage.buckets`/`storage.objects` n'existaient pas du tout** avant cette phase (0 bucket, 0 policy) — une migration Storage dédiée était donc nécessaire : `20260905184439_phase_8a_media_storage.sql`, créant uniquement le bucket privé `media` (`public=false`, `allowed_mime_types` restreint à `image/jpeg`/`image/png`/`audio/webm`/`audio/mp4`/`audio/ogg`) et deux policies `storage.objects` (`SELECT`/`INSERT to authenticated` seulement — aucune `UPDATE`/`DELETE`, aucune `anon`). Isolation par atelier vérifiée via une jointure sur `public.fiches` (sous RLS `fiches_select_member`) confirmant que le `(workshop_id, fiche_id)` encodé dans le path correspond à une fiche réelle et non supprimée — jamais une simple comparaison du segment fourni par le navigateur ; comparaisons en texte uniquement (jamais de cast `::uuid` non protégé). Aucune Edge Function : Phase 4 accorde déjà `SELECT`/`INSERT`/`UPDATE(metadata, deleted_at)` à `authenticated` sur `media_assets`, contrairement à `fiches` — le CRUD métadonnées passe par PostgREST direct, seul le Storage (upload/signature) est un sujet à part. Invariants tenus : jamais `getPublicUrl()` (URLs signées ≤ 300 s, rafraîchies avant expiration), jamais de `storage.remove()` pour une suppression normale (soft-delete `deleted_at` uniquement), jamais de retry automatique d'upload, ordre imposé upload→INSERT (création) et upload→soft-delete ancien→INSERT nouveau→signature (remplacement vocal/signature, avec restauration best-effort de l'ancien si l'INSERT échoue après le soft-delete), au plus une `voice_note` active par fiche (contrainte domaine, la DB ne l'impose que pour `signature`). Aucune persistance IndexedDB pour ce Repository (URLs signées éphémères — tout en mémoire, Phase 12 traitera l'offline). Validé en local : `db reset --local` (14 migrations dont la nouvelle), `db lint --local` 0 erreur, **61/61 groupes SQL** (T58–T61 nouveaux : bucket privé + MIME restreints, policies SELECT/INSERT authenticated exactement, aucune UPDATE/DELETE/anon, grants `media_assets` Phase 4 inchangés), script d'isolation Storage `scripts/test-phase-8a-media.mjs` (14/14 : upload/lecture/signature propriétaire, upload/lecture inter-ateliers refusés, anon refusé, soft-delete sans suppression physique, URL signée expirée → code HTTP réel observé — 400 en local, jamais présumé 403), `npx tsc -b` 0 erreur, `npm run lint` 0 nouvel avertissement, `npm test` **472/472**. Aucun déploiement distant, aucune activation de `VITE_BACKEND=supabase` (le gate exige toujours 7B+8A+8B+11A), aucune PR ouverte pour ce tour. **Nouveau statut : « Phase 8A implémentée et validée localement — PR non ouverte »**. |
| 2026-09-06 (implémentation Phase 8B) | **R (précision — migration Storage technique non anticipée) ; statut Phase 8B** | Implémentation de la Phase 8B (catalogue de modèles cloud : `SupabaseModeleRepository` CRUD `public.modeles`, extension de `SupabaseMediaRepository` pour `public.modele_medias` — domaine mémoire séparé `modeleMediaMap`, cache de signatures PARTAGÉ avec les médias fiche par `storage_path`, refresh atomique couvrant les deux collections ; `copyModeleMediaToFiche()` nouvelle primitive `MediaRepository` remplaçant la boucle historique `addFichePhoto(photo.dataUrl)` — celle-ci aurait tenté de repasser une URL HTTPS signée à `parseDataUrl()` en cloud, impossible ; `ModeleNew.tsx` corrigé : n'appelle plus `add()` au montage, vrai brouillon local). **Correction factuelle** à `02-PLAN-MIGRATION.md` (« aucune migration SQL 8B sauf éventuel GRANT ») : aucun GRANT supplémentaire n'a été nécessaire, mais une migration Storage TECHNIQUE l'a été — les 2 policies `storage.objects` de Phase 8A n'acceptaient que le path fiche ; `20260906161045_phase_8b_catalog_storage_policies.sql` DROP puis RECREATE les 2 MÊMES noms de policies (jamais 2 de plus — évite `multiple_permissive_policies`) avec une condition acceptant la branche fiche (inchangée, régression testée) OU la branche modèle (`public.modeles`, `deleted_at IS NULL`, comparaisons `::text` uniquement). Aucun changement de schéma `public.modeles`/`public.modele_medias` (Phase 2/4, inchangés). Point de sécurité vérifié avec le vrai PostgREST local (pas seulement supposé côté frontend, §29) : `listActiveModeleMedias()` utilise une jointure `modeles!inner(deleted_at)` + filtre `is("modeles.deleted_at", null)` — un média dont le modèle parent est soft-deleted n'est ni chargé ni signé, cohérent avec le refus Storage réel observé (la policy exige un modèle actif). MIME modèle restreint à `image/jpeg`/`image/png` (plus strict que le bucket global, qui reste partagé avec l'audio des vocaux fiche). Détacher une photo/patron = `DELETE` physique de la ligne `modele_medias` (GRANT Phase 4 : `SELECT`/`INSERT`/`DELETE`, jamais `UPDATE`) — jamais `storage.remove()`. Validé en local : `db reset --local` (15 migrations dont la nouvelle), `db lint --local` 0 erreur, **69/69 groupes SQL** (T62–T69 nouveaux : policies combinées toujours au nombre de 2, branche fiche préservée, branche modèle présente, couple workshop+modèle vérifié sur les deux branches, aucun cast `::uuid` non protégé, bucket/GRANT `modeles`/`modele_medias` inchangés), `scripts/test-phase-8a-media.mjs` toujours **14/14** (aucune régression), nouveau `scripts/test-phase-8b-catalog.mjs` **21/21** (isolation atelier A/B réelle, upload/lecture/signature propriétaire, cross-atelier et couple incohérent refusés, anon refusé, détachement sans suppression Storage, soft-delete du modèle parent refusant l'accès à un nouveau média — et vérification directe de la requête `!inner` contre le vrai PostgREST), `npx tsc -b` 0 erreur, `npm run lint` 0 nouvel avertissement, `npm test` **530/530**. Aucun déploiement distant, aucune activation de `VITE_BACKEND=supabase` (le gate exige toujours 7B+8A+8B+11A — 11A reste manquante), aucune PR ouverte pour ce tour. **Nouveau statut : « Phase 8B implémentée et validée localement — PR non ouverte »**. |
| 2026-09-06 (implémentation Phase 11A) | **statut Phase 11A** | Implémentation de la Phase 11A (paiements cloud minimal : `SupabasePaymentRepository` — ledger `client_payments` + vue autoritative `fiche_balances`, deux sources distinctes jamais mélangées ; `PaymentRepository.setAmount()` remplacé par `add({ficheId, amount, paidAt?, method?, note?}): Promise<Payment>`, `paymentAmountSchema` entier strictement positif ; `AvanceChampCell` devient « Total versé » + saisie locale + bouton « Ajouter », taper ne committe jamais, un seul `add()` par clic explicite, protégé contre le double-submit). **Aucune correction factuelle nécessaire cette fois** — contrairement à 9A/8A/8B, le gel n'avait anticipé aucune migration SQL et **aucune n'a été nécessaire** : `client_payments`/`fiche_balances`/leurs GRANT (Phase 2/4) suffisaient tels quels. Invariants tenus : `add()` ne committe JAMAIS de façon optimiste (INSERT confirmé serveur → row validée → commit mémoire → rafraîchissement CIBLÉ de la balance) ; un INSERT réussi suivi d'un échec du refresh de balance ne transforme jamais le versement en « non enregistré » (`getLastBalanceRefreshError()`, distinct d'un échec d'INSERT — testé explicitement, aucun second INSERT, aucun faux rollback) ; `reste` négatif (surpaiement) affiché fidèlement, `ResteChampCell` ne tronque plus à 0 (`Math.max` retiré, libellé « Trop-perçu » dédié) ; un `total_price` confirmé déclenche `refreshBalance(ficheId)` ciblé, jamais un recalcul côté client ; `client_payments` reste immuable (aucun GRANT UPDATE/DELETE ajouté, vérifié par script réel). Validé en local : `db reset --local` (15 migrations, inchangé), `db lint --local` 0 erreur, **69/69 groupes SQL** (inchangé, aucune migration touchée), `npx tsc -b` 0 erreur, `npm run lint` 0 nouvel avertissement (5 pré-existants), `npm test` **603/603**, `scripts/test-phase-8a-media.mjs` **14/14** et `scripts/test-phase-8b-catalog.mjs` **21/21** toujours verts (aucune régression), nouveau `scripts/test-phase-11a-payments.mjs` **21/21** (isolation atelier A/B réelle sur le ledger, contrainte `CHECK(amount>0)` vérifiée, ledger immuable — UPDATE/DELETE réellement refusés en RLS, surpaiement réel `total_paid=12000 → reste=-2000` vérifié contre la vraie vue SQL locale). Aucun déploiement distant, aucune activation de `VITE_BACKEND=supabase` (7B+8A+8B+11A codées mais le Gate proprement dit reste une étape séparée, non exécutée), aucune PR ouverte pour ce tour. **Nouveau statut : « Phase 11A implémentée et validée localement — PR non ouverte, distant non touché »**. |
| 2026-09-07 (implémentation Gate `VITE_BACKEND=supabase`, branchement applicatif local) | **R (activation du gate) ; statut Gate** | 7B/8A/8B/11A closed (corr. R) : le Gate lui-même est implémenté — `resolveBackend("supabase")` (`src/lib/backend.ts`) retourne désormais `"supabase"` au lieu de lever `BackendConfigurationError` (le défaut absent/vide reste `"local"`, jamais changé). `RepositoryContainer.ts` gagne un cas `"supabase"` construisant `createPhase11ACloudRepositories({ gateway, workshopId })` (clients/fiches/carnets/payments/media/modeles — un seul lot, jamais un mélange Supabase/LocalStorage) + `dispose()` (`disposePhase11ACloudRepositories()`) ; `subscriptions` reste l'adaptateur pilote `LocalStorageSubscriptionRepository` — Phase 14 n'existe pas encore, **ce n'est pas un fallback métier cloud** (documenté explicitement comme exception assumée). Fail-closed strict : `workshopId`/`supabaseGateway` absents → erreur explicite, jamais un fallback local (testé). Ni `RepositoryContainer.ts` ni `RepositoryProvider.tsx` n'importent `src/lib/supabase/client.ts`/`createSupabaseGateway()` (testabilité sans `VITE_SUPABASE_*` préservée) — seul `App.tsx` le fait, à l'intérieur de la partie protégée. **Correction structurelle nécessaire (§10/§11 de la tâche)** : `RepositoryProvider` était monté à la racine de `App.tsx` (au-dessus des 3 routes d'authentification) — au démarrage, avant restauration de session, `workshop` valait `null`, ce qui aurait tenté de construire un conteneur cloud avec un atelier absent dès qu'un Preview `VITE_BACKEND=supabase` serait activé. **Corrigé en déplaçant `RepositoryProvider` sous `RequireAuth`** (dans `ProtectedAppRoute`, jamais contourné par un faux atelier) : les routes `/connexion`, `/connexion/code`, `/connexion/atelier` ne construisent désormais plus aucun Repository métier (vérifié : `PhoneEntry`/`OtpVerify`/`WorkshopName` n'appellent `useRepositories()` nulle part), prouvé par un test qui espionne `createRepositoryContainer()` et démontre 0 appel sur `/connexion` puis 1 appel avec le VRAI `workshopId` sur une route protégée. Lifecycle : le conteneur auto-créé est disposé au démontage ET à chaque changement d'atelier (ancien disposé avant que le nouveau ne soit utilisé, jamais les deux lots vivants simultanément) ; un conteneur injecté par un test (`repositories` prop) n'est **jamais** disposé automatiquement (contrat de test préexistant intact — `RepositoryContainer.test.ts`/pages `*.test.tsx` inchangés). Validé en local : `npx tsc -b` 0 erreur, `npm run lint` 0 erreur (5 avertissements pré-existants, inchangé), `npm test` **615/615** (+10 : résolution backend, construction/fail-closed/lifecycle du conteneur Supabase, non-construction sur route d'auth vs construction avec le vrai workshopId sur route protégée), SQL **69/69** (inchangé, 0 migration touchée), `scripts/test-phase-8a-media.mjs` **14/14**, `scripts/test-phase-8b-catalog.mjs` **21/21**, `scripts/test-phase-11a-payments.mjs` **21/21** (aucune régression). Build réel `VITE_BACKEND=supabase` contre la stack Supabase **locale** (`VITE_SUPABASE_URL`/`VITE_SUPABASE_PUBLISHABLE_KEY` locaux uniquement, jamais la clé secrète pour Vite) : `npm run build` **PASS**, scan du bundle généré (`sb_secret_`/`service_role`/clé secrète) : **0 occurrence**. Aucune migration, aucun accès Supabase distant, aucune configuration Vercel, aucun commit sur `main`/`develop`, aucune PR pour ce tour (branche `feat/gate-supabase-preview`). **Nouveau statut : « Gate implémenté localement — Preview E2E distant NON exécuté, aucune activation Vercel, VITE_BACKEND=supabase toujours inactif en production »**. |
| 2026-09-07 (pivot Auth — téléphone + PIN 4 chiffres, atelier invisible) | **D5 (abandonnée) ; corr. Q (frontière service_role étendue à `tayoo-pin-auth`) ; statut pivot Auth** | Découvert BLOQUANT en cours de Preview E2E du Gate (entrée précédente) : le parcours téléphone + OTP SMS (D5, Phase 3) n'a jamais eu de fournisseur SMS réel — vérifié en direct sur `sunu-couture-dev` (`GET /auth/v1/settings` → `external.phone = false`) et en local (seuls les numéros `[auth.sms.test_otp]` fonctionnent, jamais un vrai SMS). **D5 abandonnée** au profit d'une authentification téléphone + PIN 4 chiffres gérée par une nouvelle Edge Function `tayoo-pin-auth` (`verify_jwt = false`, exception documentée : la fonction implémente elle-même une authentification stricte, seule fonction du dépôt à ce jour dans ce cas — `provision-workshop`/`create-fiche-from-draft` vérifient un JWT déjà existant, `tayoo-pin-auth` n'en a par nature aucun à vérifier avant `register`/`login`). **Architecture** (corr. Q étendue) : un PIN à 4 chiffres (10 000 combinaisons) ne devient jamais un mot de passe Supabase directement — dérivation HMAC-SHA256 (secret dédié `TAYOO_PIN_AUTH_SECRET`, jamais commité/VITE_*/navigateur/log, local via `supabase/functions/.env` gitignoré) : `phoneKey = HMAC(secret, "phone-key:v1|"+numéro)` sert de clé de recherche opaque ET base de l'email technique (`u_<phoneKey>@auth.tayoo.invalid`, jamais le numéro brut dans cette adresse) ; `technicalPassword = HMAC(secret, "password:v1|"+numéro+"|"+PIN+"|"+salt)` (salt aléatoire propre au compte) devient le mot de passe Supabase Auth réel via `admin.auth.createUser()`/`signInWithPassword()` — une VRAIE session Supabase (jamais un JWT fabriqué à la main), immédiatement compatible avec `auth.uid()`/RLS/`workshop_members`/repositories cloud existants. Nouvelle migration `20260907120000_pin_auth.sql` (16ᵉ migration) : `app_hidden.pin_auth_accounts` (`user_id`/`phone_key` unique/`password_salt` — **jamais** de numéro brut, PIN, ou mot de passe technique stockés, colonnes vérifiées absentes par test T70) et `app_hidden.pin_auth_throttle` (anti brute-force PERSISTANT — les Edge Functions étant stateless, un `Map()` ne protège rien — clé opaque HMAC scopée téléphone ET IP, jamais bruts ; verrouillage progressif 5/8/10 échecs → 1/5/30 minutes) ; 6 fonctions `app_hidden.pin_auth_*` (SECURITY DEFINER) + 6 wrappers `public.pin_auth_*_api` (SECURITY INVOKER, EXECUTE réservé à `service_role`, même schéma exact que `provision_workshop_api`) — `app_hidden` restant non exposé (`[api].schemas`), ces wrappers sont la SEULE porte, testé T71. Compensation explicite (T… script `test-pin-auth.mjs` scénario 10) : `auth.admin.createUser()` et Postgres ne partageant pas une transaction unique, un échec de l'INSERT `pin_auth_accounts` après création réussie de l'utilisateur Auth déclenche `admin.auth.admin.deleteUser()` — jamais un utilisateur orphelin silencieux, prouvé en révoquant temporairement le privilège `EXECUTE` du wrapper d'insertion. `provision_workshop_api` (Phase 3A) réutilisé tel quel pour l'auto-provisioning (idempotent par construction, aucune nouvelle porte d'écriture d'atelier) — l'atelier n'est plus jamais nommé/affiché par l'utilisateur (nom technique `Espace <8 premiers caractères de l'id>`, interne uniquement). **Front** : `AuthContextValue` remplace `initializing: boolean` + `provisionWorkshop()` par un état `status` (`initializing`/`signed_out`/`provisioning`/`ready`/`error`) et `register()`/`login()` (remplacent `sendPhoneOtp()`/`verifyPhoneOtp()`) ; nouveaux écrans `Welcome`/`PhoneEntry` (réécrit, purement navigation)/`PinCreate`/`PinConfirm`/`PinLogin` et composants `PinPad`/`PinDots` (grandes zones tactiles ≥ 64px, retour visuel par points jamais le chiffre affiché, clavier physique ET tactile via un champ réel visuellement masqué) ; `OtpVerify`/`WorkshopName`/`SupabasePhoneOtpAuthRepository` supprimés, `/connexion/atelier` redirige vers `/connexion` (compatibilité, jamais un écran mort) ; dernier numéro mémorisé localement (`localStorage`, jamais le PIN) pour l'écran « Bon retour » d'un appareil déjà reconnu. **Deux bugs réels trouvés et corrigés pendant l'implémentation** (au-delà de « les tests passent ») : (1) `PinPad` remonté pendant l'état de chargement (« On prépare ton carnet… »/« Connexion… ») retrouvait sa valeur à 4 chiffres non réinitialisée côté composant parent → boucle de resoumission infinie de `register()`/`login()`, corrigée en vidant l'état local du PIN AVANT de basculer vers l'écran de chargement (le `completedRef` interne de `PinPad` ne survit pas à un démontage/remontage, seule la valeur portée par le parent le peut) ; (2) la route protégée initialement demandée (`from`, mémorisée par `RequireAuth` avant la redirection vers `/connexion`) était silencieusement perdue à l'écran `Welcome` (jamais propagée aux `navigate()` suivants), ramenant systématiquement l'utilisateur vers `/` après connexion au lieu de la route d'origine — corrigé en relisant et repropageant `location.state.from` à chaque étape du parcours. Validé en local : `db reset --local` (16 migrations dont la nouvelle), `db lint --local` 0 erreur, **73/73 groupes SQL** (T70–T73 nouveaux), `npx tsc -b` 0 erreur, `npm run lint` 0 nouvel avertissement (5 pré-existants), `npm test` **670/670**, nouveau `scripts/test-pin-auth.mjs` **35/35** (register/login réels contre le vrai PostgREST/GoTrue local, duplicate refusé sans fuite d'email/userId/phoneKey, throttle réellement déclenché et bloquant, compensation testée), `scripts/test-phase-8a-media.mjs` **14/14**, `test-phase-8b-catalog.mjs` **21/21**, `test-phase-11a-payments.mjs` **21/21** (aucune régression — validés indépendamment, sur une base fraîchement réinitialisée, des comptes de test fixes `[auth.sms.test_otp]` propres à ces scripts). Build réel `VITE_BACKEND=supabase` local : `npm run build` **PASS**, scan bundle (`TAYOO_PIN_AUTH_SECRET`/`service_role`/`technicalPassword`) : **0 occurrence** (la seule occurrence de `sb_secret_` est le code de détection de préfixe du SDK `supabase-js` lui-même, vérifiée, pas une clé injectée). Aucune migration poussée, aucun accès Supabase distant, aucune configuration Vercel modifiée, aucune PR pour ce tour (branche `feat/auth-pin-onboarding`). **Nouveau statut : « Pivot Auth PIN implémenté et validé localement — PR non ouverte, distant non touché — Gate Preview E2E en pause jusqu'au merge + déploiement de ce pivot »**. |
